...
(async function () {
  ...
  async function fetchModulesWithLessons() {
    try {
      ...
      // Module orderIndex maps to week number (1-based)
      // If orderIndex is 0-based, we add 1
      window.lmsModules = {};

      // Sort modules by orderIndex first
      modules.sort((a, b) => a.orderIndex - b.orderIndex);

      const lessonPromises = modules.map(async (mod, idx) => {
        try {
          const lessons = await get(`/api/modules/${mod.moduleId}/lessons`);
          // Use orderIndex as week number; if it's 0, use 1-based position instead
          let weekNum = mod.orderIndex;
          if (weekNum === 0) weekNum = idx + 1;
